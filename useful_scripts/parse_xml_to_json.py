...
S_DATA_JSON = 's_data.json'
COMMON_DATA_JSON = 'common_data_artel.json'
SOURCE_XLM = 'Feed2022-02-01.xml'


def parse_feed_to_json(source_xml_file, f_result_file, s_result_file):
    """
    ...
    for index, offer in enumerate(root.iter('Ad')):
        ...
        if index == 0:
            ...
            with open(COMMON_DATA_JSON, 'w', encoding='utf-8') as f:
                json.dump(common_data, f, ensure_ascii=False, indent=4, separators=(',', ': '))
    ...
    with open('artel/address_list.json', 'w', encoding='utf-8') as f:
        f.writelines("\n".join(adress_list))
...
def main():
    parse_feed_to_json(SOURCE_XLM, F_DATA_JSON, S_DATA_JSON)
